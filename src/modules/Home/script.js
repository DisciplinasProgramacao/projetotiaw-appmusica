void async function () {
  ...
  function start() {
    pause();
    cron = setInterval(() => { timer(); if (second > 30) pause();  }, 1000);
  }
  ...
  document.getElementById("playAndPauseImg").src = '../../assets/icons/next 1.png'
  ...
  const currentMusic = (tracks) => {
    ...
    document.getElementById("name").innerText = `${current.name} - ${current.artists.map((item) => item.name)}`
    document.getElementById("image").src = current.album.images[0].url
    document.getElementById("audio").src = current.preview_url
    document.getElementById("spotifyLink").href = current.external_urls['spotify']
    isPlaying = !isPlaying;
    pause()
    reset() 
    second = 0;
  ...
  document.getElementById("playAndPause").addEventListener("click", () => {
    ...
    if (isPlaying == false) {
      ...
      song.play()
      console.log(current)
      document.getElementById("playAndPauseImg").src = '../../assets/icons/pause 1.png'
    }
  })
